...
if [[ `command -v java` ]]
    then
        ...
        tar -xf "jdk-${JAVA_VER}_linux-x64_bin.tar.gz"
...
# Copy Ozone chain related files to /root/blockchain
echo ""
echo -e "${green}Copying Ozone chain config files...${clear}"
rsync -a $GIT_DIR/ozonechain/config $BLOCKCHAIN_DIR/
...
# Creating a systemd service for Ozone chain node
echo ""
echo -e "${green}Creating a systemd service for Ozone chain node${clear}"
cd $GIT_DIR/ozonechain
source .env
if [ -z "$NODE_PUBLIC_IP" ]
    then
        echo -e "${red}Error:${clear} Public IP address of the node is not set in /root/ozonechain_quantum/ozonechain/.env"
